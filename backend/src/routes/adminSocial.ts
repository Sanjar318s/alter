...
router.post("/review/:id", (req: AuthRequest, res) => {
...
router.post("/tiktok/republish-public", (_req, res) => {
  const rows = db
    .select()
...
router.get("/youtube/start", (_req, res) => {
...
router.get("/youtube/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(`${site}/admin?social=youtube_connected`);
  } catch (err) {
...
router.get("/meta/callback", async (req, res) => {
  ...
  try {
    ...
    const site = (process.env.PUBLIC_SITE_URL || "https://altercosplay.vercel.app").replace(/\/$/, "");
    res.redirect(`${site}/admin?social=meta_connected`);
  } catch (err) {
    res.status(500).send(err instanceof Error ? err.message : "oauth error");
  }
});
...
router.get("/tiktok/callback", async (req, res) => {
  ...
  try {
    ...
    const site = (process.env.PUBLIC_SITE_URL || "https://altercosplay.vercel.app").replace(/\/$/, "");
    res.redirect(`${site}/admin?social=tiktok_connected`);
  } catch (err) {
    res.status(500).send(err instanceof Error ? err.message : "oauth error");
  }
});

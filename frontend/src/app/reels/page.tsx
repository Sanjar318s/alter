...
export default function ReelsPage() {
  ...
  useEffect(() => {
    publicationsApi
      .feed()
      .then((d) => {
        const list = (d.publications || []).map((p: any) => ({
  ...
  if (loading) {
    return <div className="p-8 font-mono text-ink-45 text-center">Загрузка рилсов…</div>;
  }
  ...
  return (
    <div className="bg-ink">
      <div
        ref={scroller}
        className="h-[calc(100dvh-57px)] overflow-y-auto snap-y snap-mandatory scroll-smooth"
      >
        {items.map((item, i) => {
          ...
          return (
            <section
              key={item.id}
              className="relative h-[calc(100dvh-57px)] snap-start snap-always flex items-center justify-center bg-stage"
            >
              <div className="relative w-full max-w-[420px] h-full max-h-[calc(100dvh-57px)] bg-ink overflow-hidden">
                {isVideo ? (
                  <video
                    src={media}
